...
function [V,D,A,difference_weight,s,o,ret] = variable3(E,A,P,s,eta,difference_weight, momentum_activated)
    ...
    while(i<m)
       ...
       V(i+1,:) =  [-1 (1/(1+exp(membrane_potential')))];
       i=i+1;
    end
   ...
   %actualizo los pesos de la primera matriz porque uso E en vez de V
    ...
    while(j <= P(2)) %cantidad de neuronas en la capa siguiente. determina la cantidad de filas en mi matriz.
        k = 1;
        x = (1-(tanh( A(j,1:P(1)+1,1) * E(1:P(1)+1)' ))^2);
        while(k <= P(1) + 1) %cantidad de neuronas en mi capa + 1. determina la cantidad de columnas en mi matriz.
            ...
            if(momentum_activated == 1)
                ...
                delta_W  = eta * beta * (1-(tanh(beta *  A(j,1:P(1)+1,1) * E(1:P(1)+1)' ))^2) * D(length(P)-1,j) * E(k) + momentum_weight; %segundo termino es momentum
    ...
    while(i < length(P)) %cantidad de matrices
        ...
        while(j <= P(i + 1)) %cantidad de neuronas en la capa siguiente. determina la cantidad de filas en mi matriz.
            k = 1;
            x = (1-(tanh( A(j,1:P(i)+1,i) * V(i,1:P(i)+1)' ))^2);
            while(k <= P(i) + 1) %cantidad de neuronas en mi capa + 1. determina la cantidad de columnas en mi matriz.
                if(momentum_activated == 1)
                    ...
                    delta_W  = eta * beta * (1-(tanh(beta *  A(j,1:P(i)+1,i) * V(i,1:P(i)+1)' ))^2) * D(length(P)-i,j) * V(i,k) + momentum_weight; %segundo termino es momentum
                    difference_weight(j, k,i) = delta_W;
                    A(j, k, i) = A(j, k, i) + delta_W;
                else
                    A(j, k, i) =A(j, k, i) + eta * beta * (1-(tanh(beta * A(j,1:P(i)+1,i) * V(i,1:P(i)+1)' ))^2) * D(length(P)-i,j) * V(i,k);
                end
                k=k+1;
                end
            j=j+1;
